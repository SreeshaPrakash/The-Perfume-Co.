...
const getOrderDetails = async (req, res) => {
    ...
    //   res.render("orderdetails", { order, overallStatus, specificAddress, user});
      ...
      const overallStatus = order.orderItems.every(item => item.orderStatus === "delivered")
      ? "delivered"
      : order.orderItems.some(item => item.orderStatus === "processing" || item.orderStatus === "shipped")
      ? "processing"
      : "pending";
...
const changeStatus = async (req, res) => {
    try {
        ...
        if (["delivered", "return request", "returned"].includes(item.orderStatus.toLowerCase())) {
        ...
        } else if (itemStatuses.some(s => ["processing", "shipped"].includes(s))) {
...
const approveReturn = async (req, res) => {
    try {
        ...
        return res.json({ success: true, message:"Return approved successfully!", updatedStatus: "returned" });
    } catch (error) {
...
const denyReturn = async (req, res) => {
    try {
        ...
        return res.json({ success: true, message: "Return request denied!", updatedStatus: "return denied" });
    } catch (error) {
